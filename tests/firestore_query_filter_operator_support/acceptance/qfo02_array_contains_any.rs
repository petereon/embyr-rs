//! QFO02 (Slice 02, US-02, Release 1) — `array-contains-any` Queries Stop
//! Crashing.
//!
//! Acceptance criteria verified here (feature-delta.md US-02):
//!   AC-QFO-04: a real `RunQuery` with an `array-contains-any` filter
//!              returns documents matching ANY listed value.
//!   AC-QFO-05: a document matching NONE of the listed values is excluded.
//!
...
/// AC-QFO-04, AC-QFO-05
///
/// @driving_port @real-io @US-02 @AC-QFO-04 @AC-QFO-05
#[tokio::test]
async fn array_contains_any_returns_documents_matching_any_listed_value() {
    let ctx = SecurityRulesFullContext::new("trailmark-prod-qfo02-array-contains-any").await;
    let mut client = FirestoreClient::new(make_channel(ctx.server.grpc_addr));

    let mut matches_urgent = std::collections::HashMap::new();
    matches_urgent.insert("tags".to_string(), array_value(vec![string_value("urgent")]));
    seed_document(&mut client, &ctx.project_id, &ctx.api_key, "posts", "post-1", matches_urgent).await;

    let mut matches_important = std::collections::HashMap::new();
    matches_important.insert("tags".to_string(), array_value(vec![string_value("important")]));
    seed_document(&mut client, &ctx.project_id, &ctx.api_key, "posts", "post-2", matches_important).await;

    let mut matches_neither = std::collections::HashMap::new();
    matches_neither.insert("tags".to_string(), array_value(vec![string_value("mundane")]));
    seed_document(&mut client, &ctx.project_id, &ctx.api_key, "posts", "post-3", matches_neither).await;
    ...
    let sq = StructuredQuery {
        ...
        r#where: Some(Filter {
            filter_type: Some(FilterType::FieldFilter(FieldFilter {
                ...
                op: FieldOp::ArrayContainsAny as i32,
                value: Some(array_value(vec![string_value("urgent"), string_value("important")])),
            })),
        }),
        ..Default::default()
    };
    ...
    assert_eq!(
        names,
        std::collections::BTreeSet::from(["post-1".to_string(), "post-2".to_string()]),
        "AC-QFO-04/05: expected exactly the 2 matching documents, got: {names:?}"
    );
}
