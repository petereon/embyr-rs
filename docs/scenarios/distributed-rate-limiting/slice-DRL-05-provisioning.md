...
Existing projects in production will not have `rate_buckets` rows after the migration. On first gRPC request to such projects, the distributed `UPDATE rate_buckets … WHERE project_id = $1 AND tokens >= 1` will return 0 rows and fall back to the per-instance bucket. This is correct and safe — the per-instance bucket enforces 1× limit. HANDLED IN MIGRATION — `0018_rate_buckets.sql` includes a backfill `INSERT INTO rate_buckets SELECT id, 1000.0, now() FROM projects WHERE status IN ('active', 'suspended') ON CONFLICT DO NOTHING`, covering all existing projects at migration time. Operators with `EMBYR_RATE_LIMIT_RPS != 1000` should run `UPDATE rate_buckets SET tokens = <new_capacity> WHERE true;` post-migration.
