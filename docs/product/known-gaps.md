...
| 3 | `Filter.or()` composite OR rejected | `crates/embyr-server/src/grpc/handler.rs:4000` | Yes — real GA Firestore feature | Degrades feature (clean `INVALID_ARGUMENT`, not a crash/wrong-data) | IN PROGRESS — `firestore-or-filter-support`; DISCUSS found the naive fix would reopen an access-control bypass in `filter_binds_field_to_uid` (ADR-031's own AND-only assumption), full security-correct fix scoped |
