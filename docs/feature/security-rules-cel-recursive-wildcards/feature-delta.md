...
|---|---|---|---|
| OQ-RW-01 | Does a recursive wildcard's own match boundary genuinely include the zero-remaining-segments case (matching its own fixed prefix's boundary document itself), per this DISCUSS's own MODERATE-HIGH-confidence, not-independently-verified recollection of real Firestore's published documentation? | Affects the exact zero-length-match acceptance criteria DISTILL writes and the parser's own boundary behavior; the fallback if wrong is narrow and contained (§ Resolution 1), not a redesign | Solution-architect (DESIGN) or acceptance-designer (DISTILL), before the parser's own zero-length-match behavior is locked in code — independently re-verify against real Firestore's own live published rules-language reference |
...
- [D2] **Confirmed real Firestore's own recursive-wildcard match boundary can include the zero-remaining-segments case** (matching the prefix's own boundary document itself) — MODERATE-HIGH confidence, not independently web-verified in this dispatch (`OQ-RW-01`).
