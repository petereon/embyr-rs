...
## Wave: DELIVER / [WHY] Upstream Issue — Regression Baseline Scope Was Narrower Than Claimed

Slice 06's own regression sweep (US-06, AC-17-97/98) found one real, genuine regression: `tests/acceptance/us_04_query_collection.rs::collection_group_query_returns_documents_from_all_matching_subcollections` — a pre-existing walking-skeleton test (predates the entire `security-rules` initiative) that issues a real `all_descendants = true` query with NO group rule defined and asserted it succeeded unrestricted, per the pre-this-feature default.

This does not contradict the DISCUSS-locked claim as literally scoped (§ Job Discovery Framing Resolution, Resolution 2's confidence note; § Handoff Package flag 7): that claim was checked against "the 133-scenario FINALIZED `security-rules` suite plus `security-rules-query-path`'s own delivered scenarios" specifically, and neither of those exercises `all_descendants = true` — confirmed true. What the claim did NOT cover was the walking-skeleton/base acceptance suite (`tests/acceptance/us_*.rs`), which predates the authorization initiative entirely and was never in DISCUSS's own search scope. This is exactly the kind of gap Slice 06's full-suite sweep exists to catch — and it did.

**Resolution**: fixed, not weakened. The pre-existing test's own domain intent (prove collection-group SQL-matching mechanics) is unrelated to authorization; it now seeds an explicit, permissive group rule (`condition_source = "true"`) before issuing its query — mirroring exactly how a real Firestore developer migrating to collection-group rules would add `match /{path=**}/events/{doc} { allow read; }` to keep an existing collection-group query working. Resolution 2's universal fail-closed default (confirmed against real Firestore's own documented behavior, § Handoff Package) is NOT weakened or special-cased for this test. Full regression re-verified green after the fix: 14/14 `us_*` targets (excluding the permanently-environmental `us_10_aws_secrets`), 24/24 `security_rules*` family targets across all four sibling features.
