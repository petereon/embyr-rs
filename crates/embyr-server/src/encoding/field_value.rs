...
mod tests {
    ...
    #[test]
    fn roundtrip_string() {
        let fv = FieldValue::String("hello".into());
        let j = field_value_to_json(&fv);
        assert_eq!(json_to_field_value(&j).unwrap(), fv);
    }
    ...
    fn roundtrip_boolean() {
        for b in [true, false] {
            let fv = FieldValue::Boolean(b);
            assert_eq!(json_to_field_value(&field_value_to_json(&fv)).unwrap(), fv);
        }
    }
    ...
    fn roundtrip_timestamp() {
        let fv = FieldValue::Timestamp(1700000000, 123456789);
        assert_eq!(json_to_field_value(&field_value_to_json(&fv)).unwrap(), fv);
    }
